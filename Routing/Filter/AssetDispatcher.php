 ...
 */
	public $priority = 8;
	protected $_rootJs = null;
	protected $_pattern = '/require\(["|\']{1}(.*?)["|\']{1}\);/m';
/**
 ...
 */
	protected function _deliverAsset(CakeResponse $response, $assetFile, $ext) {
			...
			if( $ext === 'js') {
				$content = $this->jsComplier($assetFile);
			}
			if ($ext !== 'js' and !$compressionEnabled) {
	...
	protected function jsComplier($fileName){
		$this->_rootJs = dirname($fileName).DS;	
		return $this->scanJs($fileName);
		
		  
	}
	
	
	protected function scanJs($fileName) {
	...
	protected function _replace($content) {
		...
		if(strrpos($content[1], "*")){
			$f = $this->_rootJs.$content[1].'.js';
			$files = glob($f);
			$content = '';
			foreach($files as $file){
				$content .= $this->jsComplier($file)."\n\r";
			}
			...
			if(file_exists($f)){
				return	file_get_contents($f,true);
			}else{
				return "// not exist: $content[1]";
			}
